...
provide('collectionMeta', computed(() => new Map(collections.value.map((c) => [c.id, c]))))
...
<template>
  <div class="min-h-[calc(100vh-7.3rem)] pt-10 pb-10 px-4">
    <div class="max-w-7xl mx-auto flex gap-6 article-opened">
      <aside :class="[
        'shrink-0 hidden lg:block transition-all duration-300',
        isSidebarCollapsed ? 'w-16' : 'w-72 category-left'
      ]">
        <div class="sticky top-24">
          <div class="flex items-center justify-between mb-4 px-2 category-padding">
            <h2 v-if="!isSidebarCollapsed" class="text-sm font-semibold uppercase tracking-wider text-muted">
              Категории
            </h2>
            <button @click="toggleSidebar"
              class="p-1.5 rounded-lg hover:bg-muted/50 transition-colors text-muted hover:text-foreground category-btn"
              :aria-label="isSidebarCollapsed ? 'Развернуть панель' : 'Свернуть панель'"
              :title="isSidebarCollapsed ? 'Развернуть' : 'Свернуть'">
              <svg class="w-4 h-4 transition-transform duration-300" :class="{ 'rotate-180': isSidebarCollapsed }"
          ...
          <div v-if="!isSidebarCollapsed" class="mb-4 px-2">
            <div class="relative">
              ...
              <input v-model="searchQuery" type="text" placeholder="Поиск по статьям..."
              ...
              <button v-if="searchQuery || isSearching" @click="searchQuery = ''"
          ...
          <div v-if="isSidebarCollapsed" class="space-y-1">
            <button @click="selectCollection(null)" :class="[
              ...
              <span v-if="collections.reduce((sum, c) => sum + c.articleCount, 0) > 0"
                class="absolute -top-1 -right-1 w-4 h-4 text-[10px] bg-foreground text-background rounded-full flex items-center justify-center sidebar-counter">
                {{collections.reduce((sum, c) => sum + c.articleCount, 0)}}
              </span>
            </button>
            <button v-for="coll in collections" :key="coll.id" @click="onCollapsedCategoryClick(coll.id)" :class="[
              'w-full flex items-center justify-center p-2 rounded-lg transition-colors text-sm relative',
            ...
            <div class="space-y-1 mt-2">
              <div v-for="coll in filteredCollections" :key="coll.id">
                <div class="flex items-center gap-1">
                  <button @click="toggleCollection(coll.id)"
                    class="p-1 rounded hover:bg-muted/50 transition-colors text-muted button-category-opener"
                    :aria-label="isCollectionExpanded(coll.id) ? 'Свернуть' : 'Развернуть'">
                    <svg class="w-3 h-3 transition-transform" :class="{ 'rotate-90': isCollectionExpanded(coll.id) }"
                      fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
                    </svg>
                  </button>
                  <button @click="selectCollection(coll.id)" :class="[
                ...
                <div v-if="isCollectionExpanded(coll.id)" class="ml-6 mt-1 space-y-0.5 border-l border-border pl-2">
                  <router-link v-for="article in getSidebarArticles(coll)" :key="article.id"
                    :to="`/articles/${article.id}`" class="block px-2 py-1 text-xs transition-colors truncate rounded"
                    :class="isSearchActive ? 'text-foreground hover:bg-muted/50' : 'text-muted hover:text-foreground'"
                    :title="article.title">
                    {{ article.title }}
                    <span v-if="isSearchActive && article.tags.includes('title-match')"
                      class="text-[9px] ml-1 text-muted">
                      (заголовок)
                    </span>
                  </router-link>
                  ...
                  <button v-if="hasMoreArticles(coll)" @click="loadMoreArticles(coll.id)"
                    ...
                    <span>Показать ещё {{ coll.articles.length - (expandedSidebarLimits.get(coll.id) ||
                      SIDEBAR_INITIAL_LIMIT) }}</span>
      ...
      <main class="flex-1 min-w-0">
        <div
          ...
          <div class="flex gap-2 overflow-x-auto custom-scrollbar pb-1">
            ...
            <button v-for="coll in collections" :key="coll.id" @click="selectCollectionAndCenter(coll.id, $event)"
              :title="cleanCollectionName(coll.name)"
              class="shrink-0 px-3 py-1.5 rounded-full text-xs font-medium border transition-colors" :class="getSelectedCollectionId() === coll.id
                ? 'bg-foreground text-background border-foreground'
                : 'bg-muted/10 text-muted border-border hover:text-foreground'
                ">
