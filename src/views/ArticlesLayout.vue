...
import { getArticlesStructured, cleanCollectionName } from '../services/api'
import type { CollectionWithArticles } from '../services/api'
...
const filteredCollections = computed(() => {
  if (!isSearchActive.value) return collections.value
  
  const query = searchQuery.value.toLowerCase().trim()
  return collections.value
    .map(coll => {
      const matchedArticles = coll.articles.filter(article => 
        article.title.toLowerCase().includes(query)
      )
      return { ...coll, articles: matchedArticles, articleCount: matchedArticles.length }
    })
    .filter(coll => coll.articleCount > 0)
})
          ...
          <div v-if="!isSidebarCollapsed" class="mb-4 px-2">
            <div class="relative">
              ...
              <input
                ...
                class="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-border bg-background focus:outline-none focus:ring-2 focus:ring-foreground/20 transition-all"
              />
              <button
                v-if="searchQuery"
                @click="searchQuery = ''"
              ...
              <div v-for="coll in filteredCollections" :key="coll.id">
                ...
                >
                  <router-link
                    ...
                    class="block px-2 py-1 text-xs text-muted hover:text-foreground transition-colors truncate rounded"
                    :title="article.title"
                  >
                    {{ article.title }}
                  </router-link>
