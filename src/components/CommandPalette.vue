...
const filtered = computed(() => {
  ...
  const base = q
    ...
    : commands.value
  if (SECRET_WORDS.some((w) => q.includes(w))) return [...base, secretCommand.value]
  return base
})
...
<template>
  <Transition name="palette">
    <div v-if="isOpen" ref="paletteRef" class="fixed inset-0 z-[9998] flex items-start justify-center pt-[15vh] px-4"
      @click.self="close">
      ...
      <div
        ...
        <div ref="listRef" class="max-h-[50vh] overflow-y-auto py-2" @mousemove="onListMouseMove"
          ...
          <div v-if="grouped.length === 0" class="px-5 py-8 text-center text-muted text-sm">
            Ничего не найдено
          </div>
          <div v-for="[groupName, groupCmds] in grouped" :key="groupName" class="mb-2">
            <div class="px-5 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-muted/70">
              {{ groupName }}
            </div>
            <button v-for="cmd in groupCmds" :key="cmd.id" :data-cmd-id="cmd.id"
              ...
              :class="flatFiltered[selectedIndex]?.id === cmd.id
                ...
                ">
              <span v-if="flatFiltered[selectedIndex]?.id === cmd.id"
              ...
              <div class="w-7 h-7 rounded-md bg-muted/30 flex items-center justify-center shrink-0">
                <svg v-if="FILL_ICONS.has(cmd.icon)" class="w-4 h-4 text-foreground/80" fill="currentColor"
                  viewBox="0 0 24 24" aria-hidden="true">
                  <path :d="ICONS[cmd.icon]" />
                </svg>
                <svg v-else class="w-3.5 h-3.5 text-foreground/80" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                  aria-hidden="true">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="ICONS[cmd.icon]" />
                </svg>
              </div>
              ...
              <div v-if="cmd.shortcut" class="hidden sm:flex items-center gap-1 text-[10px] font-mono text-muted/80">
                <kbd v-for="(k, i) in cmd.shortcut" :key="i" class="px-1.5 py-0.5 rounded border border-border">
                  {{ k }}
                </kbd>
              </div>
              <svg v-else class="w-3.5 h-3.5 text-muted/40 shrink-0" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
        ...
        <div
          ...
          <div class="flex items-center gap-3">
            <span class="flex items-center gap-1"><kbd class="px-1 py-0.5 rounded border border-border">↑↓</kbd>
              навигация</span>
            <span class="hidden sm:flex items-center gap-1"><kbd
                class="px-1 py-0.5 rounded border border-border">Tab</kbd> цикл</span>
            <span class="flex items-center gap-1"><kbd class="px-1 py-0.5 rounded border border-border">↵</kbd>
              выбрать</span>
          </div>
