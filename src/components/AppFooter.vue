  ...
  <footer class="border-t border-border bg-background/50 backdrop-blur-sm select-none">
    <div class="max-w-6xl mx-auto px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-muted">
      ...
      <div class="flex items-center gap-3 font-mono">
        <a
          v-if="gitTag && releaseUrl"
          :href="releaseUrl"
          target="_blank"
          rel="noopener noreferrer"
          class="px-2 py-0.5 rounded bg-muted/20 text-foreground hover:bg-muted/30 transition-colors"
        ...
        <span
          v-else-if="gitTag"
          class="px-2 py-0.5 rounded bg-muted/20 text-foreground"
          title="Релиз"
        >
          {{ gitTag }}
        </span>
        <a
          v-if="commitHash && commitHash !== 'unknown'"
          :href="commitUrl"
          target="_blank"
          rel="noopener noreferrer"
