...
const prevItem = computed<TOCItem | null>(
  () => props.items[activeIndex.value - 1] ?? null
);
const currentItem = computed<TOCItem | null>(
  () => props.items[activeIndex.value] ?? null
);
const nextItem = computed<TOCItem | null>(
  () => props.items[activeIndex.value + 1] ?? null
);
...
<template>
  ...
  >
    <div
      class="rounded-xl border border-border bg-background/90 backdrop-blur-sm shadow-lg overflow-hidden"
    >
      <button
      ...
      >
        <span
          class="text-xs font-semibold uppercase tracking-wider text-muted group-hover:text-foreground transition-colors"
        >
          Содержание
        </span>
        <svg
      ...
      <div v-if="!isExpanded" class="px-2 pb-2 space-y-0.5">
        ...
        <button
          ...
          class="block w-full text-left px-2 py-1 rounded-md text-xs text-foreground font-bold bg-muted/60 truncate"
      ...
      <div
        v-else
        ref="listRef"
        class="px-2 pb-2 max-h-[60vh] overflow-y-auto overflow-x-hidden"
      >
